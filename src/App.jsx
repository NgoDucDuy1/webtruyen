...
export default function App() {
  ...
  const displayNovels = getFilteredNovels();

  // --- ACTIONS ---
  const handleLogin = (e) => { e.preventDefault(); if (usernameInput === 'NgoDucDuy' && passwordInput === 'hthtcvnhaha') { setIsAdmin(true); setShowLoginModal(false); localStorage.setItem('novel_admin_session', 'NgoDucDuy'); alert(`Xin chào Admin!`); } else { alert('Sai tài khoản!'); } };
  const handleLogout = () => { setIsAdmin(false); localStorage.removeItem('novel_admin_session'); setView('home'); };

  const handleAddNovel = async (e) => { e.preventDefault(); if (!newNovelTitle.trim()) return; setIsSubmitting(true); try { await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'novels'), { title: newNovelTitle, author: newNovelAuthor || 'Vô danh', cover: newNovelCover || 'https://placehold.co/400x600?text=No+Cover', createdAt: Date.now(), chapterCount: 0 }); setNewNovelTitle(''); setNewNovelAuthor(''); setNewNovelCover(''); setShowAddNovelModal(false); alert('Thành công!'); } catch (e) { alert(e.message); } finally { setIsSubmitting(false); } };
  const handleAddChapter = async (e) => { e.preventDefault(); if (!newChapterTitle.trim()) return; setIsSubmitting(true); try { await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'novels', selectedNovel.id, 'chapters'), { title: newChapterTitle, content: newChapterContent, createdAt: Date.now() }); const novelRef = doc(db, 'artifacts', appId, 'public', 'data', 'novels', selectedNovel.id); await updateDoc(novelRef, { chapterCount: (chapters.length + 1) }); setNewChapterTitle(''); setNewChapterContent(''); alert('Đã đăng!'); } catch (e) { alert(e.message); } finally { setIsSubmitting(false); } };
  
  const handleUpdateNovel = async (e) => { e.preventDefault(); if (!selectedNovel) return; setIsSubmitting(true); try { const ref = doc(db, 'artifacts', appId, 'public', 'data', 'novels', selectedNovel.id); await updateDoc(ref, { title: newNovelTitle, author: newNovelAuthor, cover: newNovelCover }); setSelectedNovel(prev => ({...prev, title: newNovelTitle, author: newNovelAuthor, cover: newNovelCover})); setShowEditNovelModal(false); } catch (e) { alert(e.message); } finally { setIsSubmitting(false); } };
  const handleUpdateChapter = async (e) => { e.preventDefault(); if (!editingChapter) return; setIsSubmitting(true); try { const ref = doc(db, 'artifacts', appId, 'public', 'data', 'novels', selectedNovel.id, 'chapters', editingChapter.id); await updateDoc(ref, { title: editingChapter.title }); setEditingChapter(null); } catch (e) { alert(e.message); } finally { setIsSubmitting(false); } };

  const requestDelete = (e, type, id, title) => { e.stopPropagation(); e.preventDefault(); setDeleteModal({ show: true, type, id, title }); };
  const confirmDelete = async () => { if (!deleteModal.show) return; const { type, id } = deleteModal; setDeleteModal({ ...deleteModal, show: false }); try { if (type === 'novel') await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'novels', id)); else await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'novels', selectedNovel.id, 'chapters', id)); } catch (e) { alert(e.message); } };
    ...
    <div className={`min-h-screen font-sans transition-colors duration-300 ${themeClasses} selection:bg-blue-500 selection:text-white pb-24`}>
      ...
      {showReaderSettings && (
        <div className="fixed right-4 top-16 z-[90] animate-fade-in">
           <div className={`${cardClasses} p-4 rounded-lg shadow-2xl border w-72`}>
              <div className="flex justify-between items-center mb-3 border-b border-gray-500/20 pb-2"><h3 className="font-bold flex items-center gap-2"><Type size={16}/> Cài đặt đọc</h3><button onClick={() => setShowReaderSettings(false)}><X size={16}/></button></div>
              <div className="mb-4"><div className="flex justify-between text-sm mb-1 opacity-80"><span>Cỡ chữ: {readerSettings.fontSize}px</span></div><input type="range" min="14" max="32" step="1" value={readerSettings.fontSize} onChange={(e) => updateReaderSettings({ fontSize: parseInt(e.target.value) })} className="w-full accent-blue-600 h-2 bg-gray-500/20 rounded-lg appearance-none cursor-pointer"/></div>
              <div className="mb-4"><div className="text-sm mb-2 opacity-80">Dãn dòng</div><div className="flex gap-2 bg-gray-500/10 p-1 rounded">{[1.4, 1.6, 1.8, 2.0].map(lh => (<button key={lh} onClick={() => updateReaderSettings({ lineHeight: lh })} className={`flex-1 py-1 rounded text-xs font-medium transition-colors ${readerSettings.lineHeight === lh ? 'bg-blue-600 text-white' : 'hover:bg-gray-500/20'}`}>{lh}</button>))}</div></div>
              <div className="mb-2"><div className="text-sm mb-2 opacity-80">Font chữ</div><div className="flex gap-2"><button onClick={() => updateReaderSettings({ fontFamily: 'font-serif' })} className={`flex-1 py-2 rounded border text-sm font-serif ${readerSettings.fontFamily === 'font-serif' ? 'border-blue-600 text-blue-600 bg-blue-500/10' : 'border-gray-500/30'}`}>Serif</button><button onClick={() => updateReaderSettings({ fontFamily: 'font-sans' })} className={`flex-1 py-2 rounded border text-sm font-sans ${readerSettings.fontFamily === 'font-sans' ? 'border-blue-600 text-blue-600 bg-blue-500/10' : 'border-gray-500/30'}`}>Sans</button></div></div>
           </div>
        </div>
      )}
      ...
      {showChapterList && (
        <div className="fixed inset-0 z-[100] bg-black/50 backdrop-blur-sm flex justify-end animate-fade-in" onClick={() => setShowChapterList(false)}>
           <div className={`w-full max-w-xs h-full ${isDarkMode ? 'bg-[#171717] border-l border-[#262626]' : 'bg-white border-l border-gray-200'} shadow-2xl flex flex-col`} onClick={(e) => e.stopPropagation()}>
              <div className="p-4 border-b border-inherit flex justify-between items-center shrink-0"><h3 className="font-bold flex items-center gap-2"><List size={20}/> Danh sách chương</h3><button onClick={() => setShowChapterList(false)} className="p-1 hover:bg-gray-500/10 rounded"><X size={20}/></button></div>
              <div className="flex-1 overflow-y-auto p-2">{chapters.map((chap, idx) => (<button key={chap.id} onClick={() => readChapter(idx)} className={`w-full text-left p-3 rounded mb-1 text-sm font-medium transition-colors ${idx === currentChapterIndex ? 'bg-blue-600 text-white' : 'hover:bg-gray-500/10 opacity-80 hover:opacity-100'}`}>{chap.title}</button>))}</div>
           </div>
        </div>
      )}
      ...
      <main className="max-w-7xl mx-auto p-4 lg:py-8 min-h-[calc(100vh-64px)]">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
              {displayNovels.length === 0 ? (
                <div className="col-span-full text-center py-20 opacity-50">
                  {view === 'history' ? <><Clock size={48} className="mx-auto mb-4"/><p>Bạn chưa đọc truyện nào.</p></> : <><Book size={48} className="mx-auto mb-4"/><p>Không tìm thấy truyện.</p></>}
                </div>
              ) : (
        ...
        {view === 'detail' && selectedNovel && (
          <div className="animate-fade-in">
             <div className="mb-6"><button onClick={goHome} className="flex items-center gap-1 text-sm opacity-60 hover:opacity-100 hover:text-blue-500"><ChevronLeft size={16} /> Quay lại danh sách</button></div>
