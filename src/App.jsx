...
import { Book, ChevronLeft, ChevronRight, Menu, Search, Play, RotateCcw, Bookmark, List, Plus, Trash2, Lock, Save, User, LogOut, Settings, Image as ImageIcon, Moon, Sun, Home, AlertTriangle, X, SkipForward, Edit, Headphones, Volume2, Mic } from 'lucide-react';
...
export default function App() {
  ...
  // --- STATE AUDIO (ADVANCED UI LIKE IMAGE) ---
  ...
  useEffect(() => {
    const loadVoices = () => {
      ...
      // Ưu tiên giọng Microsoft/Edge nếu có (như trong ảnh)
      const bestVoice = vnVoices.find(v => v.name.includes('Microsoft') || v.name.includes('Neural')) || vnVoices[0] || availVoices[0];
      
      setVoices(vnVoices.length > 0 ? vnVoices : availVoices);
      setTtsConfig(prev => ({ ...prev, voice: bestVoice }));
    };
  ...
  // Stop audio when changing chapters or view
  useEffect(() => {
     if (isSpeaking) {
  ...
  const handleTTSPlay = () => {
      ...
      const utterance = new SpeechSynthesisUtterance(chapters[currentChapterIndex].content);
      if (ttsConfig.voice) utterance.voice = ttsConfig.voice;
  ...
  return (
    <div className={`min-h-screen font-sans transition-colors duration-300 ${themeClasses} selection:bg-blue-500 selection:text-white`}>
      ...
      {/* --- TTS MODAL (GIAO DIỆN GIỐNG ẢNH) --- */}
      {showTTSModal && (
        <div className="fixed inset-0 z-[90] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in" onClick={() => setShowTTSModal(false)}>
            <div className={`w-full max-w-md ${isDarkMode ? 'bg-[#262626] text-white' : 'bg-white text-gray-900'} rounded-2xl shadow-2xl overflow-hidden border border-gray-500/20`} onClick={e => e.stopPropagation()}>
                {/* Header Modal */}
                <div className="p-4 border-b border-gray-500/20 flex justify-between items-center">
                    <h3 className="font-bold text-lg flex items-center gap-2"><Headphones size={20} className="text-blue-500"/> Text To Speech</h3>
                    <button onClick={() => setShowTTSModal(false)} className="p-1 hover:bg-gray-500/20 rounded-full"><X size={20}/></button>
                </div>

                {/* Body Modal */}
                <div className="p-6 flex flex-col gap-6">
                    {/* Engine Info (Giả lập giống ảnh) */}
                    <div className="flex justify-between items-center text-sm">
                        <span className="opacity-60">Engine</span>
                        <span className="opacity-80 flex items-center gap-1">Microsoft (Online) <ChevronRight size={14}/></span>
                    </div>

                    {/* Language Info */}
                    <div className="flex justify-between items-center text-sm">
                        <span className="opacity-60">Ngôn ngữ</span>
                        <span className="opacity-80 flex items-center gap-1">Tiếng Việt (Việt Nam) <ChevronRight size={14}/></span>
                    </div>

                    {/* Voice Select */}
                    <div>
                        <div className="flex justify-between items-center mb-2 text-sm">
                            <span className="opacity-60">Giọng đọc</span>
                        </div>
                        <div className={`p-3 rounded-xl border ${isDarkMode ? 'bg-[#171717] border-[#404040]' : 'bg-gray-100 border-gray-300'} flex items-center justify-between`}>
                             <select 
                                className="bg-transparent w-full outline-none appearance-none font-medium"
                                value={ttsConfig.voice?.name || ''}
                                onChange={(e) => {
                                    const v = voices.find(vo => vo.name === e.target.value);
                                    setTtsConfig({...ttsConfig, voice: v});
                                    if(isSpeaking) handleTTSStop();
                                }}
                             >
                                {voices.map(v => (
                                    <option key={v.name} value={v.name} className="text-black">{v.name.replace('Microsoft', '').replace('Online', '').trim()} (Online)</option>
                                ))}
                             </select>
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-2 text-sm">
                            ...
                            <span className="font-mono">{ttsConfig.rate}x</span>
                        </div>
                        <input 
                            type="range" min="0.5" max="2" step="0.1" 
                            value={ttsConfig.rate} 
                            onChange={(e) => {
                                setTtsConfig({...ttsConfig, rate: parseFloat(e.target.value)});
                                if(isSpeaking) handleTTSStop();
                            }}
                            className="w-full h-1 bg-gray-500/30 rounded-lg appearance-none cursor-pointer accent-blue-500"
                        />
                    </div>
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-2 text-sm">
                            <span className="opacity-60">Độ cao</span>
                            <span className="font-mono">{Math.round(ttsConfig.pitch * 100)}</span>
                        </div>
                        <input 
                            type="range" min="0.5" max="2" step="0.1" 
                            value={ttsConfig.pitch} 
                            onChange={(e) => {
                                setTtsConfig({...ttsConfig, pitch: parseFloat(e.target.value)});
                                if(isSpeaking) handleTTSStop();
                            }}
                            className="w-full h-1 bg-gray-500/30 rounded-lg appearance-none cursor-pointer accent-blue-500"
                        />
                ...
                {/* Footer Action */}
                <div className="p-4 border-t border-gray-500/20 bg-gray-500/5">
                    {isSpeaking ? (
                         <button onClick={handleTTSStop} className="w-full py-3 rounded-xl bg-red-600 hover:bg-red-700 text-white font-bold shadow-lg flex items-center justify-center gap-2">
                             <div className="w-2 h-2 bg-white rounded-full animate-ping"></div> Dừng đọc
                         </button>
                    ) : (
                         <button onClick={handleTTSPlay} className="w-full py-3 rounded-xl border border-gray-500/30 hover:bg-gray-500/10 font-bold transition-colors">
                             Nghe thử
                         </button>
                    )}
                </div>
        ...
        <div className="flex items-center gap-3 text-sm font-medium">
          ...
          {view === 'reader' && (
              <button 
                ...
                className={`p-2 rounded-full transition-colors ${isSpeaking ? 'bg-blue-600 text-white animate-pulse' : (isDarkMode ? 'hover:bg-[#262626] text-blue-400' : 'hover:bg-gray-100 text-blue-600')}`}
                title="Cấu hình Audio"
              >
                <Headphones size={20} />
              </button>
          )}
        ...
        {view === 'reader' && selectedNovel && chapters[currentChapterIndex] && (
          <div className="max-w-3xl mx-auto animate-fade-in pb-20">
             <div className="sticky top-16 z-30 flex flex-col md:flex-row justify-between items-center mb-4 border-b border-inherit bg-inherit py-3 opacity-95 gap-3">
                ...
                {/* REMOVED INLINE AUDIO PLAYER - REPLACED BY HEADER BUTTON & MODAL */}
             </div>
